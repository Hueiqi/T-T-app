...
class _PlanDetailScreenState extends State<PlanDetailScreen> {
  ...
  void _sharePlan() {
    final plan = _plan;
    final workoutList = plan.weeklyWorkouts
        .map((w) => '• ${w.day}: ${w.focus} (${w.durationMinutes} min)')
        .join('\n');
    final mealList = plan.meals
        .map((m) => '• ${m.meal}: ${m.calories} kcal')
        .join('\n');

    final text = '''
Check out my fitness plan: ${plan.title}

${plan.tagline}

📋 Details:
• Difficulty: ${plan.difficulty}
• ${plan.workoutsPerWeek} workouts/week
• ${plan.dailyCalories} kcal/day
• Goal: ${plan.estimatedGoalWeeks} weeks
...
''';
  ...
  Widget _buildProgressHeader(FitnessPlan plan) {
    ...
    final daysRemaining = _plan.estimatedGoalWeeks > 0
        ? (_plan.estimatedGoalWeeks - _currentWeek) * 7
        : 0;
    final goalLabels = {
    ...
    final goalLabel = user != null ? (goalLabels[user.fitnessGoal] ?? 'Fitness') : 'Fitness';
    final hasTarget = user != null && user.targetWeightKg != null;
    final weightDiff = hasTarget ? (user!.weight - user.targetWeightKg!).abs() : 0.0;
        ...
        children: [
          ...
          if (hasTarget || user != null)
            Container(
              ...
              child: Row(
                children: [
                  if (hasTarget) ...[
                    Icon(Icons.monitor_weight_outlined, color: Colors.white.withValues(alpha: 0.9), size: 18),
                    const SizedBox(width: 6),
                    Text(
                      '${user!.weight.round()} → ${user.targetWeightKg!.round()} kg',
                    ...
                    Container(
                      ...
                      child: Text(
                        '${weightDiff.toStringAsFixed(1)} kg to go',
                    ...
                    const SizedBox(width: 6),
                    Text(
                      '${user!.weight.round()} kg · ${goalLabel}',
                      style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600),
                    ),
                  ],
                  const Spacer(),
          ...
          Row(
            ...
            children: [
              ...
              Text(
                '$_currentWeek / ${plan.estimatedGoalWeeks} weeks',
                style: TextStyle(
                  color: Colors.white.withValues(alpha: 0.7),
                  fontSize: 11,
                ),
  ...
  Widget _buildStatsRow(FitnessPlan plan, NutritionProvider nutrition) {
    ...
    final double calRatio = calGoal > 0 ? (calCurrent / calGoal).clamp(0.0, 1.0).toDouble() : 0.0;
    return Padding(
...
Widget _buildMacroSection(FitnessPlan plan, NutritionProvider nutrition) {
  ...
  Widget _macroRow(String label, double current, double goal, Color color) {
    final ratio = goal > 0 ? (current / goal).clamp(0.0, 1.0).toDouble() : 0.0;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: isToday
                        ? AppTheme.primaryColor
                        : (isWorkoutDay
                            ? (isDark ? AppTheme.workoutDayBgDark : AppTheme.workoutDayBg)
                            : (isDark ? AppTheme.restDayBgDark : AppTheme.restDayBg)),
                    borderRadius: BorderRadius.circular(10),
                    border: isWorkoutDay && !isToday
                        ? Border.all(
                            color: AppTheme.workoutDayAccent.withValues(alpha: isDark ? 0.5 : 0.3),
                            width: 1.2,
                          )
                        : null,
                  ),
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          fontWeight: isToday
                              ? FontWeight.bold
                              : (isWorkoutDay ? FontWeight.w600 : FontWeight.w500),
                          color: isToday
                              ? Colors.white
                              : (isWorkoutDay
                                  ? AppTheme.workoutDayAccent
